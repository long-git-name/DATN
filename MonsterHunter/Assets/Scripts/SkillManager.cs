...
public class SkillManager : Singleton<SkillManager>
{
    ...
    {
        SkillDataWrapper skillDataWrapper = new SkillDataWrapper(skillCollecteds);
        Prefs.skillData = JsonUtility.ToJson(skillDataWrapper);
    }

    public void LoadSkillData()
    {
        if (PlayerPrefs.HasKey(PrefsConst.SKILL_DATA_KEY))
        {
            string json = Prefs.skillData;
            SkillDataWrapper skillDataWrapper = JsonUtility.FromJson<SkillDataWrapper>(json);
            skillCollecteds = skillDataWrapper.ToDictionary();
        }
    }
